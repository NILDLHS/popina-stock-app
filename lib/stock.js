...
// Suggestions de mise en fabrication pour le laboratoire : pour chaque produit ayant une recette,
// cumule le manque (seuil configure - stock actuel) sur toutes les boutiques en dessous de leur
// seuil. Base uniquement sur les seuils deja en place (pas d'historique de vente) : simple,
// previsible, et ne declenche rien tout seul - une suggestion a valider manuellement.
function getProductionSuggestions(tenantId) {
  const rows = db.prepare(`
    SELECT s.name as site_name, p.id as product_id, p.name as product_name, p.sku, u.code as unit_code, t.min_quantity,
           COALESCE((SELECT SUM(m.quantity) FROM stock_movements m WHERE m.site_id = s.id AND m.product_id = p.id), 0) as current_qty
    FROM stock_thresholds t
    JOIN sites s ON s.id = t.site_id
    JOIN products p ON p.id = t.product_id
    JOIN units u ON u.id = p.unit_id
    WHERE s.tenant_id = ? AND s.is_active = 1 AND s.type != 'PRODUCTION' AND p.is_active = 1
      AND EXISTS (SELECT 1 FROM recipe_items ri WHERE ri.product_id = p.id)
  `).all(tenantId);

  const byProduct = new Map();
  for (const r of rows) {
    if (r.current_qty >= r.min_quantity) continue;
    const missing = r.min_quantity - r.current_qty;
    if (!byProduct.has(r.product_id)) {
      byProduct.set(r.product_id, { product_id: r.product_id, product_name: r.product_name, sku: r.sku, unit_code: r.unit_code, total_missing: 0, sites: [] });
    }
    const entry = byProduct.get(r.product_id);
    entry.total_missing += missing;
    entry.sites.push({ site_name: r.site_name, missing });
  }
  return [...byProduct.values()].sort((a, b) => b.total_missing - a.total_missing);
}
...
// Dernier prix d'achat connu pour un produit (centimes), toutes commandes fournisseurs confondues -
// aucune saisie manuelle requise, on reutilise les prix deja renseignes sur les commandes recues.
// Retourne null si le produit n'a jamais ete commande avec un prix.
function getLatestUnitPrice(productId) {
  const row = db.prepare(`
    SELECT pol.unit_price FROM purchase_order_lines pol
    JOIN purchase_orders po ON po.id = pol.purchase_order_id
    WHERE pol.product_id = ? AND pol.unit_price IS NOT NULL
    ORDER BY po.created_at DESC, pol.rowid DESC LIMIT 1
  `).get(productId);
  return row ? row.unit_price : null;
}

// Cout matiere estime d'un produit fini/semi-fini : cumule quantite d'ingredient x dernier prix
// d'achat connu. Si un ingredient n'a jamais ete achete avec un prix, le cout est signale incomplet
// (plutot que silencieusement sous-estime).
function getMaterialCost(productId) {
  const recipe = getRecipe(productId);
  let totalCents = 0;
  let complete = true;
  const lines = recipe.map((item) => {
    const unitPrice = getLatestUnitPrice(item.ingredient_product_id);
    if (unitPrice === null) { complete = false; return { ...item, unit_price: null, line_cost: null }; }
    const lineCost = unitPrice * item.quantity;
    totalCents += lineCost;
    return { ...item, unit_price: unitPrice, line_cost: lineCost };
  });
  return { lines, totalCents, complete: complete && recipe.length > 0 };
}
